...
def get_config_path(profile: str = 'default'):
    ...
    config_path = home_path.joinpath(".config/webbrowser2").joinpath(profile)
...
class BrowserProc(object):
    ...
    def _create_window(self, socket_id: int, com_pipe: object,
        ...
        scroll.set_policy(self._gtk.PolicyType.AUTOMATIC,self._gtk.PolicyType.AUTOMATIC)
...
class MainWindow(object):
    ...
    def __init__(self, com_pipe: object, com_dict: object,
                 uri: str = 'about:blank', profile: str = 'default'):
        """ Initialize the process.
        ...
        self._config_path = get_config_path(profile)
        self._config_file = self._config_path.joinpath('config')
        ...
        if self._socket_file.is_socket():
            try:
                ...
                self._socket.send(json_dumps(('new-tab', uri)).encode())
                self._socket.close()
    ...
    def run(self):
        ...
        if not self._socket._closed: self._gtk.main()
...
if __name__ == '__main__':
    ...
    main_p = Process(target=run_main, args=(main_ppipe, main_dict),
                     kwargs={'profile': args.profile, 'uri': args.uri})
    main_p.start()
    print("main_loop pid: ", main_p.pid)

    main(main_cpipe, main_dict)

    # run_main(main_ppipe, main_dict)
    # from socket_process import MainWindow
    # main = MainWindow(main_ppipe, main_dict)
    # main.run()

    main_p.join(1)
    if main_p.is_alive():
        print("Terminating main: {main_p.pid}".format(**locals()))
        main_p.terminate()
