...
class BrowserProc(object):
    ...
    def _create_plug(self, view_dict: dict):
        ...
        plug.add(view_dict.webview)
        view_dict.webview.load_uri('http://reddit.com/r/bobdylan')
...
class MainWindow(object):
    ...
    def _make_tab(self, uri: str = 'about:blank', switch_to: bool = False,
        ...
        # self.t = [main_pipe, child_pipe, com_dict]
        # socket = self._gtk.Socket()
        # label = self._gtk.Label('test')
        # self._tabs.insert_page(socket, label, 0)
        # socket_id = socket.get_id()
        # socket.show_all()
        # label.show_all()
        # main_pipe.send(('open-uri', 'http://reddit.com/r/bobdylan'))
    ...
    def _add_tab(self, com_tup: tuple, uri: str = 'about:blank',
        ...
        com_tup[0].send(('open-uri', 'http://reddit.com/r/bobdylan'))
